...
struct ServiceDemand {
    ...
    capacity_requirements: Arc<[ServiceRequirementRef]>,
    response_requirements: Arc<[ServiceRequirementRef]>,
    cancellation_requirements: Arc<[ServiceRequirementRef]>,
...
#[derive(Clone, Debug, PartialEq, Eq)]
struct ServicePlanCandidate {
    context: u128,
    assignment_fingerprint: u128,
    cores: Arc<[ServiceCoreObservation]>,
    classes: Arc<[ServiceClassObservation]>,
}

#[derive(Clone, Debug, PartialEq, Eq)]
pub struct ServiceCoreObservation {
    ordinal: u16,
    cycle_units: u64,
    maximum_cycle_units: u64,
}

impl ServiceCoreObservation {
    #[must_use]
    ...
    #[must_use]
    pub const fn cycle_units(&self) -> u64 {
        self.cycle_units
    }

    #[must_use]
    pub const fn maximum_cycle_units(&self) -> u64 {
...
impl ServiceClassObservation {
    ...
    #[must_use]
    pub const fn kind(&self) -> ServiceClassKind {
        self.kind
    }
    ...
    #[must_use]
    pub const fn activation_units(&self) -> u64 {
        self.activation_units
    }

    #[must_use]
    pub const fn maximum_response_units(&self) -> u64 {
        self.maximum_response_units
    }

    #[must_use]
    pub const fn maximum_delay_units(&self) -> u64 {
        self.maximum_delay_units
    }

    #[must_use]
    pub const fn maximum_cancellation_response_units(&self) -> u64 {
        self.maximum_cancellation_response_units
    }

    #[must_use]
    pub const fn maximum_cancellation_delay_units(&self) -> u64 {
        self.maximum_cancellation_delay_units
    }

    #[must_use]
    pub const fn fallback_units(&self) -> u64 {
        self.fallback_units
    }

    #[must_use]
    pub fn fallback_order(&self) -> &[u128] {
        &self.fallback_order
    }
}

#[derive(Clone, Debug, PartialEq, Eq)]
pub struct ServicePlanObservation {
    fingerprint: u128,
    context_identity: u128,
...
impl ServicePlanObservation {
    #[must_use]
    pub const fn phase_schema(&self) -> &'static str {
        "wrela.service-plan.v1"
    }
    ...
    #[must_use]
    pub fn cores(&self) -> &[ServiceCoreObservation] {
        &self.cores
    }

    #[must_use]
    pub fn classes(&self) -> &[ServiceClassObservation] {
        &self.classes
    }
}

#[derive(Clone)]
pub(crate) struct VerifiedServicePlan {
    assignment: Arc<VerifiedWholeImageAssignment>,
    candidate: ServicePlanCandidate,
    fingerprint: u128,
    _verified: Verified,
}

impl fmt::Debug for VerifiedServicePlan {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        formatter
            .debug_struct("VerifiedServicePlan")
            .field("fingerprint", &format_args!("{:032x}", self.fingerprint))
            .finish_non_exhaustive()
    }
}

impl PartialEq for VerifiedServicePlan {
    fn eq(&self, other: &Self) -> bool {
        self.fingerprint == other.fingerprint
            && self.assignment.fingerprint == other.assignment.fingerprint
    }
}

impl Eq for VerifiedServicePlan {}

impl VerifiedServicePlan {
    pub(crate) const fn fingerprint(&self) -> u128 {
        self.fingerprint
    }

    pub(crate) fn observation(&self) -> ServicePlanObservation {
        ServicePlanObservation {
            fingerprint: self.fingerprint,
            context_identity: self.candidate.context,
            whole_image_assignment_fingerprint: self.candidate.assignment_fingerprint,
            cores: Arc::clone(&self.candidate.cores),
            classes: Arc::clone(&self.candidate.classes),
        }
    }
}

#[derive(Clone, Debug, PartialEq, Eq)]
pub(crate) enum ServicePlanOutcome {
    Plan(VerifiedServicePlan),
...
impl VerifiedServiceConflict {
    pub(crate) const fn code(&self) -> ServiceConflictCode {
        self.code
    }
    ...
    pub(crate) const fn required_units(&self) -> u64 {
        self.required_units
    }

    pub(crate) const fn available_units(&self) -> u64 {
        self.available_units
    }
}

impl ServiceConflictCode {
    pub(crate) const fn tag(self) -> u8 {
        match self {
            Self::CycleCapacity => 1,
            Self::MaximumDelay => 2,
            Self::CancellationDelay => 3,
            Self::SimultaneousMaximums => 4,
        }
    }
}

#[derive(Clone, Debug, PartialEq, Eq)]
enum ServiceSolveOutcome {
    Plan(ServicePlanCandidate),
    Conflict(VerifiedServiceConflict),
}

const MAX_SERVICE_SEARCH_STATES: u64 = 1_000_000;

fn solve_service_problem(
    problem: &ServiceProblem,
    cancellation: &Cancellation,
) -> Result<ServiceSolveOutcome, PlanningFailure> {
    solve_service_problem_with_limit(problem, cancellation, MAX_SERVICE_SEARCH_STATES)
}

fn solve_service_problem_with_limit(
    problem: &ServiceProblem,
    cancellation: &Cancellation,
    maximum_states: u64,
) -> Result<ServiceSolveOutcome, PlanningFailure> {
    checkpoint(cancellation)?;
    let cores: Cow<'_, [ServiceCoreResource]> = if problem
        .cores
        .windows(2)
        .all(|pair| pair[0].ordinal < pair[1].ordinal)
    {
        Cow::Borrowed(&problem.cores)
    } else {
        let mut cores = problem.cores.clone();
        cores.sort_by_key(|core| core.ordinal);
        Cow::Owned(cores)
    };
    if cores.is_empty()
        ...
        || cores.iter().any(|core| core.maximum_cycle_units == 0)
    {
        return defect("Service Plan has malformed or duplicate core resources");
    }
    let demands: Cow<'_, [ServiceDemand]> = if problem
        .demands
        .windows(2)
        .all(|pair| (pair[0].kind, pair[0].identity) < (pair[1].kind, pair[1].identity))
    {
        Cow::Borrowed(&problem.demands)
    } else {
    ...
    if demands
        ...
        || demands.iter().any(|demand| {
            demand.identity == 0
                ...
                || [
                    demand.capacity_requirements.as_ref(),
                    demand.response_requirements.as_ref(),
                    demand.cancellation_requirements.as_ref(),
                ]
                .iter()
                .any(|evidence| {
                    evidence.is_empty()
                        ...
                        || evidence
                            .iter()
                            .any(|reference| !demand.requirements.contains(reference))
                        || !evidence
                            .iter()
                            .any(|reference| reference.identity == demand.requirement)
                })
                ...
                || demand.maximum_concurrency > demand.maximum_simultaneous_release
                || demand.activation_units == 0
                || demand.maximum_non_preemptive_units == 0
                || demand.maximum_non_preemptive_units > demand.activation_units
                || demand.maximum_delay_units == 0
                || demand.maximum_cancellation_delay_units == 0
        })
    {
        return defect("Service Plan has malformed demand or placement relationships");
    }
    ...
    loop {
        ...
        if states > maximum_states {
            return defect("Service Plan quota search exhausted its authenticated finite bound");
        }
        let quotas = digits
            .iter()
            .map(|digit| u32::try_from(digit + 1).unwrap_or(u32::MAX))
            .collect::<Vec<_>>();
        if let Some(plan) = service_candidate_if_feasible(problem, &cores, &demands, &quotas) {
            return Ok(ServiceSolveOutcome::Plan(plan));
        }
        if !advance_service_digits(&mut digits, &radices) {
            break;
        }
    }
...
fn advance_service_digits(digits: &mut [usize], radices: &[usize]) -> bool {
...
fn service_candidate_if_feasible(
    problem: &ServiceProblem,
    cores: &[ServiceCoreResource],
    demands: &[ServiceDemand],
    quotas: &[u32],
) -> Option<ServicePlanCandidate> {
    let mut planned_cores = Vec::with_capacity(cores.len());
    let mut core_bounds = Vec::with_capacity(cores.len());
    for core in cores {
        let cycle = demands
            .iter()
            .zip(quotas)
            .filter(|(demand, _)| demand.core == core.ordinal)
            .try_fold(0_u64, |total, (demand, quota)| {
                total.checked_add(demand.activation_units.checked_mul(u64::from(*quota))?)
            })?;
        if cycle > core.maximum_cycle_units {
            return None;
        }
        let blocker = demands
            .iter()
            .filter(|demand| demand.core == core.ordinal)
            .map(|demand| demand.maximum_non_preemptive_units)
            .max()
            .unwrap_or(0);
        core_bounds.push((core.ordinal, cycle, blocker));
        planned_cores.push(ServiceCoreObservation {
            ordinal: core.ordinal,
            cycle_units: cycle,
            maximum_cycle_units: core.maximum_cycle_units,
        });
    }
    let mut classes = Vec::with_capacity(demands.len());
    for (index, (demand, quota)) in demands.iter().zip(quotas).enumerate() {
        let (_, cycle, blocker) = core_bounds
            .iter()
            .find(|(ordinal, _, _)| *ordinal == demand.core)?;
        let rounds = u64::from(demand.maximum_simultaneous_release).div_ceil(u64::from(*quota));
        let response = blocker.checked_add(rounds.checked_mul(*cycle)?)?;
        let cancellation_response = response.checked_add(demand.cancellation_work_units)?;
        if response > demand.maximum_delay_units
...
fn service_conflict(
    ...
    for mask in 1_usize..(1_usize << demands.len()) {
        ...
        if let Some(candidate) =
            ...
            && best
                .as_ref()
                .is_none_or(|current| service_conflict_precedes(&candidate, current))
        {
            best = Some(candidate);
        }
    }
...
fn service_conflict_precedes(
    candidate: &VerifiedServiceConflict,
    current: &VerifiedServiceConflict,
) -> bool {
    let mut candidate_identities = candidate
        .requirements
        .iter()
        .map(|reference| reference.identity)
        .collect::<Vec<_>>();
    ...
    candidate.requirements.len() < current.requirements.len()
        || (candidate.requirements.len() == current.requirements.len()
            && (candidate.code < current.code
                || (candidate.code == current.code
                    && (candidate_identities
                        .cmp(&current_identities)
                        .then_with(|| candidate.required_units.cmp(&current.required_units))
                        .then_with(|| candidate.available_units.cmp(&current.available_units))
                        .is_lt()))))
}
...
fn classify_service_conflict_subset(
        ...
        if required > core.maximum_cycle_units {
            let requirements = canonical_service_requirements(
                demands
                    .iter()
                    .filter(|demand| demand.core == core.ordinal)
                    .flat_map(|demand| demand.capacity_requirements.iter().copied()),
            );
    ...
    loop {
        ...
        *states = states.saturating_add(1);
        ...
        if cores
            .iter()
            .all(|core| cycles[&core.ordinal] <= core.maximum_cycle_units)
        {
    ...
    let mut exact_bound_conflict: Option<VerifiedServiceConflict> = None;
    for (index, demand) in demands.iter().enumerate() {
        for (code, required_units, available_units, requirement_evidence) in [
            (
            ...
            let candidate = VerifiedServiceConflict {
                code,
                context: problem.context,
                assignment_fingerprint: problem.assignment_fingerprint,
                requirements: canonical_service_requirements(
                    demands
                        .iter()
                        .filter(|other| other.core == demand.core)
                        .flat_map(|other| other.capacity_requirements.iter().copied())
                        .chain(requirement_evidence.iter().copied()),
                ),
                required_units,
                available_units,
            };
            if exact_bound_conflict
                .as_ref()
                .is_none_or(|current| service_conflict_precedes(&candidate, current))
            {
                exact_bound_conflict = Some(candidate);
            }
        }
    }
    if exact_bound_conflict.is_some() {
        return Ok(exact_bound_conflict);
    }
    Ok(Some(VerifiedServiceConflict {
        code: ServiceConflictCode::SimultaneousMaximums,
        context: problem.context,
        assignment_fingerprint: problem.assignment_fingerprint,
        requirements: canonical_service_requirements(demands.iter().flat_map(|demand| {
            demand
                .response_requirements
                .iter()
                .chain(demand.cancellation_requirements.iter())
                .copied()
        })),
...
fn verify_service_candidate(
    ...
    checkpoint(cancellation)?;
    if candidate.context != problem.context
        || candidate.assignment_fingerprint != problem.assignment_fingerprint
    {
        return defect("Service Plan mixes compilation context or Assignment authority");
    }
    let cores: Cow<'_, [ServiceCoreResource]> = if problem
        .cores
        .windows(2)
        .all(|pair| pair[0].ordinal < pair[1].ordinal)
    {
        ...
        let mut ordered = problem.cores.clone();
        ordered.sort_by_key(|core| core.ordinal);
        Cow::Owned(ordered)
    };
    let demands: Cow<'_, [ServiceDemand]> = if problem
        .demands
        ...
        .all(|pair| (pair[0].kind, pair[0].identity) < (pair[1].kind, pair[1].identity))
    {
        ...
        let mut ordered = problem.demands.clone();
        ordered.sort_by_key(|demand| (demand.kind, demand.identity));
        Cow::Owned(ordered)
    };
    if candidate.classes.len() != demands.len()
        || candidate.cores.len() != cores.len()
        || candidate
            .classes
            .iter()
            .zip(demands.iter())
    ...
    let quotas = candidate
        .classes
        .iter()
        .map(|entry| entry.quota)
        .collect::<Vec<_>>();
    let Some((expected_cores, expected_bounds)) =
        verifier_service_bounds(&cores, &demands, &quotas)
    else {
        return defect("Service Plan is oversubscribed or carries a false bound");
    };
    if candidate.cores.as_ref() != expected_cores.as_slice() {
        return defect("Service Plan core cycles do not reconstruct");
    }
    for (index, ((planned, demand), (response, cancellation_response))) in candidate
        .classes
        .iter()
        .zip(demands.iter())
        .zip(&expected_bounds)
    ...
    loop {
        ...
        if verifier_service_bounds(&cores, &demands, &prior).is_some() {
            return defect("Service Plan did not choose the canonical quota vector");
        }
        ...
        if !advanced {
            return defect("Service Plan quota vector is outside the finite search");
        }
    }
    Ok(())
}

type VerifierServiceBounds = (Vec<ServiceCoreObservation>, Vec<(u64, u64)>);

fn verifier_service_bounds(
    cores: &[ServiceCoreResource],
    demands: &[ServiceDemand],
    quotas: &[u32],
) -> Option<VerifierServiceBounds> {
    if quotas.len() != demands.len() {
        return None;
    }
    let mut core_bounds = Vec::with_capacity(cores.len());
    let mut planned_cores = Vec::with_capacity(cores.len());
    for core in cores {
        ...
        for (demand, quota) in demands.iter().zip(quotas) {
            if demand.core == core.ordinal {
                cycle =
                    cycle.checked_add(demand.activation_units.checked_mul(u64::from(*quota))?)?;
            }
        }
        if cycle > core.maximum_cycle_units {
            return None;
        ...
        let blocking = demands
            ...
            .max()
            .unwrap_or(0);
        core_bounds.push((core.ordinal, cycle, blocking));
        planned_cores.push(ServiceCoreObservation {
            ordinal: core.ordinal,
            cycle_units: cycle,
            maximum_cycle_units: core.maximum_cycle_units,
    ...
    for (demand, quota) in demands.iter().zip(quotas) {
        ...
        let rounds = u64::from(demand.maximum_simultaneous_release)
            .checked_add(u64::from(*quota).saturating_sub(1))?
            / u64::from(*quota);
        let response = blocking.checked_add(rounds.checked_mul(*cycle)?)?;
        let cancellation_response = response.checked_add(demand.cancellation_work_units)?;
        if response > demand.maximum_delay_units
            || cancellation_response > demand.maximum_cancellation_delay_units
        {
        ...
        bounds.push((response, cancellation_response));
    }
    Some((planned_cores, bounds))
}

fn verify_service_conflict(
    problem: &ServiceProblem,
    candidate: &VerifiedServiceConflict,
    cancellation: &Cancellation,
) -> Result<(), PlanningFailure> {
...
fn verifier_classify_service_subset(
    ...
    for (index, demand) in demands.iter().enumerate() {
        for (code, required_units, available_units, evidence) in [
            (
                ServiceConflictCode::MaximumDelay,
                minimum_delay[index],
                demand.maximum_delay_units,
            ...
            let mut requirements = demands
                ...
                .filter(|other| other.core == demand.core)
                .flat_map(|other| other.capacity_requirements.iter().copied())
                .chain(evidence.iter().copied())
                .collect::<Vec<_>>();
            requirements.sort_unstable();
            requirements.dedup();
            bound_candidates.push(VerifiedServiceConflict {
                code,
                context: problem.context,
                assignment_fingerprint: problem.assignment_fingerprint,
                requirements: requirements.into(),
    ...
    if !bound_candidates.is_empty() {
        return Ok(bound_candidates.into_iter().min_by(|left, right| {
            let mut left_identities = left
                .requirements
                .iter()
                .map(|reference| reference.identity)
                .collect::<Vec<_>>();
            left_identities.sort_unstable();
            let mut right_identities = right
                .requirements
                .iter()
                .map(|reference| reference.identity)
                .collect::<Vec<_>>();
            right_identities.sort_unstable();
            (
...
#[cfg(test)]
fn independently_enumerate_service_puzzle(problem: &ServiceProblem) -> ServiceSolveOutcome {
    let mut cores = problem.cores.clone();
    cores.sort_by_key(|core| core.ordinal);
    let mut demands = problem.demands.clone();
    ...
    for ordinal in 0..combinations {
        ...
        for (index, (demand, quota)) in demands.iter().zip(&quotas).enumerate() {
            ...
            let response = blocking
                + u64::from(demand.maximum_simultaneous_release).div_ceil(u64::from(*quota))
                    * cycles[&demand.core];
            let cancellation = response + demand.cancellation_work_units;
            valid &= response <= demand.maximum_delay_units
        ...
        if valid {
            return ServiceSolveOutcome::Plan(ServicePlanCandidate {
                context: problem.context,
                assignment_fingerprint: problem.assignment_fingerprint,
                cores: cores
                    .iter()
                    .map(|core| ServiceCoreObservation {
                        ordinal: core.ordinal,
                        cycle_units: cycles[&core.ordinal],
                        maximum_cycle_units: core.maximum_cycle_units,
                    })
                    .collect::<Vec<_>>()
                    .into(),
...
fn independently_classify_service_conflict(
    ...
    for mask in 1_usize..(1_usize << demands.len()) {
        let subset = demands
            ...
            .map(|(_, demand)| demand)
            .collect::<Vec<_>>();
        if let Some((core, required, available)) = cores.iter().find_map(|core| {
            let required = subset
                .iter()
                .filter(|demand| demand.core == core.ordinal)
                .map(|demand| demand.activation_units)
                .sum::<u64>();
            (required > core.maximum_cycle_units).then_some((
                core.ordinal,
                required,
                core.maximum_cycle_units,
            ))
        }) {
            let mut requirements = subset
                .iter()
                .filter(|demand| demand.core == core)
                .flat_map(|demand| demand.capacity_requirements.iter().copied())
                .collect::<Vec<_>>();
            requirements.sort_unstable();
            requirements.dedup();
        ...
        let mut found_exact_bound = false;
        for (index, demand) in subset.iter().enumerate() {
            for (code, required_units, available_units, evidence) in [
                (
                    ServiceConflictCode::MaximumDelay,
                    minimum_delay[index],
...
#[cfg(test)]
fn tiny_service_problem(
    name: &'static str,
    kind: ServiceClassKind,
    capacity: u64,
    maximum_delay: u64,
    cancellation_work: u64,
    maximum_cancellation_delay: u64,
) -> ServiceProblem {
    let requirements = synthetic_service_requirements(1, 20 + u128::from(kind.tag()));
    ServiceProblem {
        name,
        context: 1,
        ...
        cores: vec![ServiceCoreResource {
            ordinal: 0,
            maximum_cycle_units: capacity,
        }],
        demands: vec![ServiceDemand {
            identity: 10 + u128::from(kind.tag()),
            requirement: 20 + u128::from(kind.tag()),
            requirements: Arc::clone(&requirements),
            capacity_requirements: Arc::clone(&requirements),
            response_requirements: Arc::clone(&requirements),
            cancellation_requirements: requirements,
            kind,
            core: 0,
            maximum_simultaneous_release: 1,
            maximum_concurrency: 1,
            activation_units: 4,
            maximum_non_preemptive_units: 2,
            maximum_delay_units: maximum_delay,
            cancellation_work_units: cancellation_work,
            maximum_cancellation_delay_units: maximum_cancellation_delay,
        }],
    }
}

#[cfg(test)]
fn named_tiny_service_puzzles() -> Vec<ServiceProblem> {
    let mut puzzles = Vec::new();
    for (kind, exact_name, short_name) in [
        (
            ServiceClassKind::Ingress,
            "ingress_exact_fit",
            "ingress_one_short",
        ),
        (
            ServiceClassKind::ActorTurn,
            "actor_turn_exact_fit",
            "actor_turn_one_short",
        ),
        (
            ServiceClassKind::GroupChild,
            "group_child_exact_fit",
            "group_child_one_short",
        ),
        (
            ServiceClassKind::Driver,
            "driver_exact_fit",
            "driver_one_short",
        ),
        (
            ServiceClassKind::Cleanup,
            "cleanup_exact_fit",
            "cleanup_one_short",
        ),
    ] {
        puzzles.push(tiny_service_problem(exact_name, kind, 4, 6, 0, 6));
        puzzles.push(tiny_service_problem(short_name, kind, 3, 6, 0, 6));
    }
    puzzles.push(tiny_service_problem(
        "deadline_exact_fit",
        ServiceClassKind::ActorTurn,
        4,
        6,
    ...
    let demand = |identity, kind, core, release, concurrency, activation, delay| -> ServiceDemand {
        let requirements = synthetic_service_requirements(1, identity + 100);
        ServiceDemand {
            identity,
...
fn service_plan_fingerprint(candidate: &ServicePlanCandidate) -> u128 {
    let mut hash = Xxh3::new();
    hash.update(b"wrela.service-plan.v1");
    hash.update(&candidate.context.to_le_bytes());
    hash.update(&candidate.assignment_fingerprint.to_le_bytes());
    for core in candidate.cores.iter() {
        hash.update(&u128::from(core.ordinal).to_le_bytes());
        hash.update(&u128::from(core.cycle_units).to_le_bytes());
        hash.update(&u128::from(core.maximum_cycle_units).to_le_bytes());
    }
    for class in candidate.classes.iter() {
        for value in [
            class.identity,
            class.requirement,
...
fn verifier_service_plan_fingerprint(candidate: &ServicePlanCandidate) -> u128 {
    let mut verifier = Xxh3::new();
    verifier.update(b"wrela.service-plan.v1");
    verifier.update(&candidate.context.to_le_bytes());
    verifier.update(&candidate.assignment_fingerprint.to_le_bytes());
    for core in candidate.cores.iter() {
        verifier.update(&u128::from(core.ordinal).to_le_bytes());
        verifier.update(&u128::from(core.cycle_units).to_le_bytes());
        verifier.update(&u128::from(core.maximum_cycle_units).to_le_bytes());
    }
    for class in candidate.classes.iter() {
        for value in [
            class.identity,
...
fn service_demand_identity(kind: ServiceClassKind, subject: u128) -> u128 {
    producer_hash(
        b"wrela.service-demand.identity.v1",
        &[u128::from(kind.tag()), subject],
    )
}

fn actor_turn_service_identity(actor: u128, handler: u128) -> u128 {
    producer_hash(b"wrela.service-demand.actor-handler.v1", &[actor, handler])
}

fn service_release(bound: u64) -> Result<u32, PlanningFailure> {
    u32::try_from(bound)
...
fn production_service_problem(
    assignment: &VerifiedWholeImageAssignment,
    cancellation: &Cancellation,
) -> Result<ServiceProblem, PlanningFailure> {
    checkpoint(cancellation)?;
    ...
    for actor in flow.actors() {
        ...
        let handler_cores = actor
            .handlers()
            .map(|handler| placements.get(&handler).copied())
            .collect::<Option<Vec<_>>>()
            .ok_or_else(|| {
                PlanningFailure::Defect(Arc::from(
                    "Service Plan Actor handler has no verified placement",
                ))
            })?;
        let Some(core) = handler_cores.first().copied() else {
            continue;
        };
        ...
        let mailbox = flow_requirements
            ...
            .find(|requirement| {
                requirement.actor() == actor.identity()
                    && requirement.kind() == FlowRequirementKind::MailboxCapacity
            })
            .ok_or_else(|| {
                PlanningFailure::Defect(Arc::from(
                    "Service Plan Actor is missing its Mailbox Requirement",
                ))
            })?;
        let turn = flow_requirements
            .iter()
            .copied()
            .find(|requirement| {
                requirement.actor() == actor.identity()
                    && requirement.kind() == FlowRequirementKind::TurnLease
            })
            .ok_or_else(|| {
                PlanningFailure::Defect(Arc::from(
        ...
        let ingress_requirements = canonical_service_requirements([
        ...
        demands.push(ServiceDemand {
            ...
            requirements: Arc::clone(&ingress_requirements),
            capacity_requirements: Arc::clone(&ingress_requirements),
            response_requirements: Arc::clone(&ingress_requirements),
            cancellation_requirements: ingress_requirements,
            kind: ServiceClassKind::Ingress,
            core,
            maximum_simultaneous_release: release,
            maximum_concurrency: release,
            activation_units: ingress_units,
            maximum_non_preemptive_units: ingress_units,
        ...
        for handler in actor.handlers() {
            ...
            let turn_units = u64::from(service.actor_turn_units)
                .checked_add(work.maximum_logical_work_units)
                .ok_or_else(|| {
                    PlanningFailure::Defect(Arc::from("Actor Turn service cost overflow"))
                })?;
            let turn_requirements = canonical_service_requirements([
                architecture_evidence,
                flow_evidence(mailbox),
                flow_evidence(turn),
                flow_evidence(placement),
            ...
            demands.push(ServiceDemand {
                identity: actor_turn_service_identity(actor.identity(), handler),
                requirement: turn.reference().identity(),
                requirements: Arc::clone(&turn_requirements),
                capacity_requirements: Arc::clone(&turn_requirements),
                response_requirements: Arc::clone(&turn_requirements),
                cancellation_requirements: turn_requirements,
    ...
    for group in flow.groups() {
        checkpoint(cancellation)?;
        let core = placements.get(&group.handler()).copied().ok_or_else(|| {
            PlanningFailure::Defect(Arc::from(
                "Service Plan Group handler has no verified placement",
            ))
        })?;
        let child = flow_requirements
            .iter()
            .copied()
            .find(|requirement| {
                requirement.kind() == FlowRequirementKind::GroupChildActivationBound
                    && requirement.site() == Some(group.identity())
            })
            .ok_or_else(|| {
                PlanningFailure::Defect(Arc::from(
        ...
        let placement = flow_requirements
            ...
            .find(|requirement| {
                requirement.kind() == FlowRequirementKind::PermanentCorePlacement
                    && requirement.actor() == group.actor()
            })
            .ok_or_else(|| {
                PlanningFailure::Defect(Arc::from(
                    "Service Plan Group is missing its placement Requirement",
                ))
            })?;
        let work_bound = flow_requirements
            .iter()
            .copied()
            .find(|requirement| {
            ...
            .ok_or_else(|| {
                PlanningFailure::Defect(Arc::from(
                    "Service Plan Group is missing its work-bound Requirement",
                ))
            })?;
        let deadline_evidence = flow_requirements
            .iter()
            .copied()
            .filter(|requirement| {
                matches!(
                    ...
                    || requirement.site() == group.deadline_authority())
            })
            .map(flow_evidence)
            .collect::<Vec<_>>();
        let cancellation_evidence = flow_requirements
            .iter()
            .copied()
            .filter(|requirement| {
                (matches!(
                    ...
                    || (requirement.kind() == FlowRequirementKind::CancellationCheckpoint
                        && requirement
                            .site()
                            .is_some_and(|site| group.has_cancellation_checkpoint(site)))
            })
            .map(flow_evidence)
            .collect::<Vec<_>>();
        let capacity_requirements = canonical_service_requirements([
            architecture_evidence,
            flow_evidence(child),
            flow_evidence(placement),
            flow_evidence(work_bound),
        ]);
        let response_requirements = canonical_service_requirements(
            capacity_requirements
                .iter()
                .copied()
                .chain(deadline_evidence.iter().copied()),
        );
        let cancellation_requirements = canonical_service_requirements(
            capacity_requirements
                .iter()
                .copied()
                .chain(cancellation_evidence.iter().copied()),
        );
        let group_requirements = canonical_service_requirements(
            response_requirements
                .iter()
                .copied()
                .chain(cancellation_requirements.iter().copied()),
        );
        ...
        let work = u64::from(service.group_child_units)
            ...
            .ok_or_else(|| PlanningFailure::Defect(Arc::from("Group service cost overflow")))?;
        let cleanup_work = u64::try_from(group.cleanup_action_count())
            .unwrap_or(u64::MAX)
            .checked_mul(u64::from(service.cleanup_units))
            .ok_or_else(|| PlanningFailure::Defect(Arc::from("cleanup service cost overflow")))?;
        let cancellation_work = group
            .maximum_uninterrupted_work_units()
            .checked_add(u64::from(service.cancellation_checkpoint_units))
            .and_then(|value| value.checked_add(cleanup_work))
            .ok_or_else(|| {
                PlanningFailure::Defect(Arc::from("cancellation service cost overflow"))
            })?;
        demands.push(ServiceDemand {
            identity: service_demand_identity(ServiceClassKind::GroupChild, group.identity()),
            requirement: child.reference().identity(),
            requirements: Arc::clone(&group_requirements),
            capacity_requirements: Arc::clone(&capacity_requirements),
            response_requirements: Arc::clone(&response_requirements),
            cancellation_requirements: Arc::clone(&cancellation_requirements),
            kind: ServiceClassKind::GroupChild,
            core,
            maximum_simultaneous_release: release,
            maximum_concurrency: release,
            activation_units: work,
            maximum_non_preemptive_units: group.maximum_uninterrupted_work_units().min(work),
            maximum_delay_units: group.deadline_slack().unwrap_or(default_delay),
            cancellation_work_units: cancellation_work,
            maximum_cancellation_delay_units: default_cancellation_delay,
        });
        if group.cleanup_action_count() > 0 {
            let cleanup = flow_requirements
                .iter()
                .copied()
                .find(|requirement| {
                    requirement.kind() == FlowRequirementKind::GroupCleanupOrder
                        && requirement.site() == Some(group.identity())
                })
                .ok_or_else(|| {
                    PlanningFailure::Defect(Arc::from(
                        "Service Plan Group is missing its cleanup Requirement",
                    ))
                })?;
            let cleanup_release = u32::try_from(group.cleanup_action_count()).map_err(|_| {
                PlanningFailure::Defect(Arc::from("cleanup release bound exceeds schema"))
            })?;
            let cleanup_capacity_requirements = canonical_service_requirements([
                architecture_evidence,
                flow_evidence(cleanup),
                flow_evidence(placement),
            ]);
            let cleanup_response_requirements = canonical_service_requirements(
                cleanup_capacity_requirements
                    .iter()
                    .copied()
                    .chain(deadline_evidence.iter().copied()),
            );
            let cleanup_cancellation_requirements = canonical_service_requirements(
                cleanup_capacity_requirements
                    .iter()
                    .copied()
                    .chain(cancellation_evidence.iter().copied()),
            );
            let cleanup_requirements = canonical_service_requirements(
                cleanup_response_requirements
                    .iter()
                    .copied()
                    .chain(cleanup_cancellation_requirements.iter().copied()),
            );
            demands.push(ServiceDemand {
                identity: service_demand_identity(ServiceClassKind::Cleanup, group.identity()),
                requirement: cleanup.reference().identity(),
                requirements: Arc::clone(&cleanup_requirements),
                capacity_requirements: cleanup_capacity_requirements,
                response_requirements: cleanup_response_requirements,
                cancellation_requirements: cleanup_cancellation_requirements,
                kind: ServiceClassKind::Cleanup,
                core,
                maximum_simultaneous_release: cleanup_release,
                maximum_concurrency: 1,
                activation_units: u64::from(service.cleanup_units),
                maximum_non_preemptive_units: u64::from(service.cleanup_units),
                maximum_delay_units: group.deadline_slack().unwrap_or(default_delay),
                cancellation_work_units: u64::from(service.cancellation_checkpoint_units),
                maximum_cancellation_delay_units: default_cancellation_delay,
            });
    ...
    for role in foundation.generated_roles.iter().filter(|role| {
        matches!(
            role.kind,
            GeneratedRoleKind::DisplayDriver
                | GeneratedRoleKind::InputDriver
        ...
        checkpoint(cancellation)?;
        let core = placements
            .get(&role.executable.identity)
            .copied()
            .ok_or_else(|| {
                PlanningFailure::Defect(Arc::from("Service Plan Driver has no verified placement"))
            })?;
        let requirement = foundation
            .requirements
            .iter()
            .find(|requirement| {
                requirement.subject == RequirementOwner::GeneratedRole(role.reference)
            ...
            .ok_or_else(|| {
                PlanningFailure::Defect(Arc::from(
                    "Service Plan Driver is missing its realization Requirement",
                ))
            })?;
        let driver_requirements = canonical_service_requirements([
            architecture_evidence,
            ServiceRequirementRef {
        ...
        demands.push(ServiceDemand {
            ...
            requirement: requirement.reference.identity,
            requirements: Arc::clone(&driver_requirements),
            capacity_requirements: Arc::clone(&driver_requirements),
            response_requirements: Arc::clone(&driver_requirements),
            cancellation_requirements: driver_requirements,
            kind: ServiceClassKind::Driver,
            core,
            maximum_simultaneous_release: 1,
            maximum_concurrency: 1,
            activation_units: u64::from(service.driver_units),
            maximum_non_preemptive_units: u64::from(service.driver_units),
            maximum_delay_units: default_delay,
            cancellation_work_units: u64::from(service.cancellation_checkpoint_units),
            maximum_cancellation_delay_units: default_cancellation_delay,
        });
...
fn verify_actor_turn_demand_work(
    ...
    for actor in flow.actors() {
        ...
        let mailbox = flow_requirements
            ...
            .ok_or_else(|| {
                PlanningFailure::Defect(Arc::from(
                    "Service work verifier is missing an Actor mailbox Requirement",
                ))
            })?;
        let turn = flow_requirements
            .iter()
            ...
            .ok_or_else(|| {
                PlanningFailure::Defect(Arc::from(
                    "Service work verifier is missing an Actor Turn Requirement",
                ))
            })?;
        let placement = flow_requirements
            .iter()
            ...
            .ok_or_else(|| {
                PlanningFailure::Defect(Arc::from(
                    "Service work verifier is missing an Actor placement Requirement",
                ))
            })?;
        let reference = |requirement: ImagePlanningFlowRequirement<'_>| ServiceRequirementRef {
            source: ServiceRequirementSource::Flow,
            context: foundation.context,
            identity: requirement.reference().identity(),
            current_meaning: requirement.reference().current_meaning(),
        ...
        for handler in actor.handlers() {
            ...
            let expected_units = baseline
                .checked_add(work.maximum_logical_work_units)
                .ok_or_else(|| {
                    PlanningFailure::Defect(Arc::from("Actor Turn verifier overflow"))
                })?;
            let identity = actor_turn_service_identity(actor.identity(), handler);
            ...
            let demand = problem
                ...
                .ok_or_else(|| {
                    PlanningFailure::Defect(Arc::from(
                        "Service work verifier is missing a handler-specific Actor Turn demand",
                    ))
                })?;
            ...
            if demand.requirement != turn.reference().identity()
                || demand.requirements.as_ref() != expected_references
                || demand.capacity_requirements.as_ref() != expected_references
                || demand.response_requirements.as_ref() != expected_references
                || demand.cancellation_requirements.as_ref() != expected_references
                || demand.core
                    != placements.get(&handler).copied().ok_or_else(|| {
                        PlanningFailure::Defect(Arc::from(
                            "Service work verifier cannot reconstruct handler placement",
                        ))
                    })?
                || demand.maximum_simultaneous_release != service_release(actor.mailbox_capacity())?
                || demand.maximum_concurrency != u32::from(actor.max_active_turns())
                || demand.activation_units != expected_units
                || demand.maximum_non_preemptive_units != expected_units
                || demand.maximum_delay_units != u64::from(service.maximum_cycle_units)
                || demand.cancellation_work_units
                    != u64::from(service.cancellation_checkpoint_units)
                || demand.maximum_cancellation_delay_units
                    != u64::from(service.maximum_cancellation_delay_units)
            {
                return defect(
                    "Handler-specific Actor Turn service or evidence does not reconstruct",
                );
            }
        }
    }
    let actual_identities = problem
        .demands
        .iter()
        .filter(|demand| demand.kind == ServiceClassKind::ActorTurn)
        .map(|demand| demand.identity)
        .collect::<Vec<_>>();
    if actual_identities.len() != expected_identities.len()
        || actual_identities.into_iter().collect::<BTreeSet<_>>() != expected_identities
    {
        return defect("Service work verifier found extra or repointed Actor Turn service");
    }
    Ok(())
}
    ...
    pub(crate) fn service_plan(
        ...
        match solve_service_problem(&problem, cancellation)? {
            ServiceSolveOutcome::Plan(candidate) => {
                ...
                Ok(ServicePlanOutcome::Plan(plan))
            }
            ServiceSolveOutcome::Conflict(conflict) => {
                verify_service_conflict(&problem, &conflict, cancellation)?;
...
#[cfg(test)]
fn verify_service_plan(
    plan: &VerifiedServicePlan,
    cancellation: &Cancellation,
) -> Result<(), PlanningFailure> {
    checkpoint(cancellation)?;
    if plan.candidate.assignment_fingerprint != plan.assignment.fingerprint {
...
fn verify_service_plan_against(
    problem: &ServiceProblem,
    plan: &VerifiedServicePlan,
    cancellation: &Cancellation,
) -> Result<(), PlanningFailure> {
    verify_service_candidate(problem, &plan.candidate, cancellation)?;
    if plan.fingerprint != verifier_service_plan_fingerprint(&plan.candidate) {
        return defect("Service Plan fingerprint does not reconstruct");
    }
    Ok(())
}
...
"#,
    ...
    fn assignment_fixture() -> VerifiedWholeImageAssignment {
        assignment_from_foundation(fixture(Root::Image))
    }

    fn assignment_from_foundation(
        foundation: VerifiedPlanningFoundation,
    ) -> VerifiedWholeImageAssignment {
...
"#,
    ...
    #[test]
    fn named_tiny_service_puzzles_match_the_independent_brute_force_checker() {
        let puzzles = named_tiny_service_puzzles();
        assert_eq!(
            puzzles
                .iter()
                .map(|puzzle| puzzle.name)
        ...
        for puzzle in puzzles {
            let production = solve_service_problem(&puzzle, &Cancellation::new())
                .expect("bounded production service search completes");
            let independent = independently_enumerate_service_puzzle(&puzzle);
            assert_eq!(
                production, independent,
    ...
    fn service_corruption_fixture() -> (ServiceProblem, ServicePlanCandidate) {
        let demand = |identity, kind| {
            let requirements = synthetic_service_requirements(7, identity + 100);
            ServiceDemand {
                identity,
                requirement: identity + 100,
                requirements: Arc::clone(&requirements),
                ...
                maximum_cancellation_delay_units: 21,
            }
        };
        let problem = ServiceProblem {
            ...
            demands: vec![
                demand(1, ServiceClassKind::Ingress),
                demand(2, ServiceClassKind::ActorTurn),
            ],
        };
        let ServiceSolveOutcome::Plan(candidate) =
            solve_service_problem(&problem, &Cancellation::new()).unwrap()
        else {
            panic!("corruption fixture must admit");
        };
        (problem, candidate)
    }

    #[test]
    fn service_verifier_rejects_missing_oversubscribed_invalid_false_noncanonical_and_mixed_plans()
    {
        let (problem, canonical) = service_corruption_fixture();
        verify_service_candidate(&problem, &canonical, &Cancellation::new()).unwrap();

        let mut missing = canonical.clone();
        missing.classes = missing.classes[..1].to_vec().into();
        assert!(matches!(
            verify_service_candidate(&problem, &missing, &Cancellation::new()),
            Err(PlanningFailure::Defect(_))
        ));

        let mut oversubscribed = canonical.clone();
        let mut classes = oversubscribed.classes.to_vec();
        classes.iter_mut().for_each(|class| class.quota = 2);
        oversubscribed.classes = classes.into();
        assert!(matches!(
            verify_service_candidate(&problem, &oversubscribed, &Cancellation::new()),
            Err(PlanningFailure::Defect(_))
        ));

        let mut invalid_placement = canonical.clone();
        Arc::make_mut(&mut invalid_placement.classes)[0].core = 1;
        assert!(matches!(
            verify_service_candidate(&problem, &invalid_placement, &Cancellation::new()),
            Err(PlanningFailure::Defect(_))
        ));

        let mut false_bound = canonical.clone();
        ...
        quotas[1] = 2;
        noncanonical =
            service_candidate_if_feasible(&problem, &problem.cores, &problem.demands, &quotas)
                .expect("later quota vector remains feasible");
        assert!(matches!(
            verify_service_candidate(&problem, &noncanonical, &Cancellation::new()),
            Err(PlanningFailure::Defect(_))
        ));

        let mut mixed_context = canonical;
        mixed_context.context ^= 1;
        assert!(matches!(
            verify_service_candidate(&problem, &mixed_context, &Cancellation::new()),
            Err(PlanningFailure::Defect(_))
        ));

        let assignment = Arc::new(assignment_fixture());
        let ServicePlanOutcome::Plan(mut signed) = ImagePlanningModule
            .service_plan(assignment, &Cancellation::new())
            .unwrap()
        else {
            panic!("verified assignment has feasible service");
        };
        signed.fingerprint ^= 1;
        assert!(matches!(
            verify_service_plan(&signed, &Cancellation::new()),
            Err(PlanningFailure::Defect(_))
        ));
    }

    #[test]
    fn handler_specific_service_verifier_rejects_duplicate_missing_repointed_and_corrupted_demands()
    {
        let assignment = assignment_from_foundation(fixture_from_source(
            "src/image.wr",
            br#"@actor
struct Worker:
...
"#,
        ...
        let canonical = production_service_problem(&assignment, &Cancellation::new()).unwrap();
        verify_actor_turn_demand_work(&assignment, &canonical, &Cancellation::new()).unwrap();
        let actor_turns = canonical
            .demands
            .iter()
            .filter(|demand| demand.kind == ServiceClassKind::ActorTurn)
            .cloned()
            .collect::<Vec<_>>();
        assert_eq!(actor_turns.len(), 2);

        let mut duplicate = canonical.clone();
        duplicate.demands.push(actor_turns[0].clone());
        assert!(matches!(
            verify_actor_turn_demand_work(&assignment, &duplicate, &Cancellation::new()),
            Err(PlanningFailure::Defect(_))
        ));
        ...
        missing
            .demands
            .retain(|demand| demand.identity != actor_turns[0].identity);
        assert!(matches!(
            verify_actor_turn_demand_work(&assignment, &missing, &Cancellation::new()),
            Err(PlanningFailure::Defect(_))
        ));

        let mut repointed = canonical.clone();
        repointed
            .demands
            .iter_mut()
            .find(|demand| demand.identity == actor_turns[0].identity)
            .unwrap()
            .identity ^= 1;
        assert!(matches!(
            verify_actor_turn_demand_work(&assignment, &repointed, &Cancellation::new()),
            Err(PlanningFailure::Defect(_))
        ));

        let mut corrupted = canonical;
        let demand = corrupted
            .demands
            .iter_mut()
            .find(|demand| demand.identity == actor_turns[0].identity)
            .unwrap();
        demand.activation_units += 1;
        assert!(matches!(
            verify_actor_turn_demand_work(&assignment, &corrupted, &Cancellation::new()),
            Err(PlanningFailure::Defect(_))
        ));
    }

    #[test]
    fn service_conflict_evidence_has_exact_bounds_and_cancellation_publishes_nothing() {
        let puzzle = tiny_service_problem("one_short", ServiceClassKind::Cleanup, 3, 6, 0, 6);
        let ServiceSolveOutcome::Conflict(conflict) =
            solve_service_problem(&puzzle, &Cancellation::new()).unwrap()
        else {
            panic!("one-short service must conflict");
        };
        assert_eq!(conflict.required_units, 4);
        assert_eq!(conflict.available_units, 3);
        let mut false_conflict = conflict;
        false_conflict.required_units -= 1;
        assert!(matches!(
            verify_service_conflict(&puzzle, &false_conflict, &Cancellation::new()),
            Err(PlanningFailure::Defect(_))
        ));
        ...
        cancellation.cancel();
        assert!(matches!(
            solve_service_problem(&puzzle, &cancellation),
            Err(PlanningFailure::Cancelled)
        ));

        let mut exhausted = tiny_service_problem(
            "bounded_exhaustion",
            ServiceClassKind::ActorTurn,
            8,
            1,
    ...
    fn service_concurrency_caps_quota_and_fallback_only_borrows_fitting_activations() {
        let demand = |identity, activation, release, concurrency| {
            let requirements = synthetic_service_requirements(1, identity + 100);
            ServiceDemand {
                identity,
                requirement: identity + 100,
    ...
    #[test]
    fn handler_specific_cheap_and_expensive_service_is_exact_and_conflicts_one_unit_short() {
        let demand = |identity, activation| {
            let requirements = synthetic_service_requirements(1, identity + 100);
            ServiceDemand {
                identity,
                requirement: identity + 100,
                ...
                activation_units: activation,
                maximum_non_preemptive_units: activation,
                maximum_delay_units: 21,
                cancellation_work_units: 0,
                maximum_cancellation_delay_units: 21,
            }
        };
        let mut problem = ServiceProblem {
            name: "handler_specific_exact_fit",
            context: 1,
            assignment_fingerprint: 2,
            cores: vec![ServiceCoreResource {
                ordinal: 0,
                maximum_cycle_units: 11,
            }],
            demands: vec![demand(1, 1), demand(2, 10)],
        };
        let ServiceSolveOutcome::Plan(plan) =
            solve_service_problem(&problem, &Cancellation::new()).unwrap()
        else {
            panic!("cheap and expensive handler costs must fit their exact combined capacity");
        };
        assert_eq!(
            plan.classes
                .iter()
        ...
        problem.cores[0].maximum_cycle_units = 10;
        let ServiceSolveOutcome::Conflict(conflict) =
            solve_service_problem(&problem, &Cancellation::new()).unwrap()
        else {
            panic!("handler-specific service must conflict one authenticated unit short");
        };
        assert_eq!(conflict.required_units, 11);
        assert_eq!(conflict.available_units, 10);
    }
    ...
    fn service_conflict_is_bound_to_exact_canonical_inclusion_minimal_requirements() {
        ...
        let evidence = |architecture_identity, flow_identity| {
            canonical_service_requirements([
                ServiceRequirementRef {
                    source: ServiceRequirementSource::Architecture,
                    context: 7,
                    identity: architecture_identity,
                    current_meaning: architecture_identity + 1,
                },
                ServiceRequirementRef {
                    source: ServiceRequirementSource::Flow,
                    context: 7,
                    identity: flow_identity,
                    current_meaning: flow_identity + 1,
                },
            ])
        };
        let lex_demand = |identity, requirements: Arc<[ServiceRequirementRef]>| ServiceDemand {
            identity,
            requirement: requirements
                .iter()
                .find(|reference| reference.source == ServiceRequirementSource::Flow)
                .unwrap()
                .identity,
            requirements: Arc::clone(&requirements),
            capacity_requirements: Arc::clone(&requirements),
        ...
        let identity_rank_problem = ServiceProblem {
            name: "requirement_identity_ranking",
            context: 7,
            assignment_fingerprint: 9,
            cores: vec![ServiceCoreResource {
                ordinal: 0,
                maximum_cycle_units: 3,
            }],
            demands: vec![
                lex_demand(1, evidence(100, 1)),
                lex_demand(2, evidence(2, 99)),
            ],
        };
        let ServiceSolveOutcome::Conflict(identity_ranked) =
            solve_service_problem(&identity_rank_problem, &Cancellation::new()).unwrap()
        else {
            panic!("each single demand exceeds capacity");
        };
        assert_eq!(
            identity_ranked
                .requirements
    ...
    #[test]
    fn service_conflicts_retain_only_the_requirement_facts_used_by_the_selected_contradiction() {
        let requirement = |source, identity| ServiceRequirementRef {
            source,
            context: 7,
            identity,
            current_meaning: identity + 100,
        };
        let architecture = requirement(ServiceRequirementSource::Architecture, 1);
        let core_work = requirement(ServiceRequirementSource::Core, 2);
        let service = requirement(ServiceRequirementSource::Flow, 3);
        let placement = requirement(ServiceRequirementSource::Flow, 4);
        let work_bound = requirement(ServiceRequirementSource::Flow, 5);
        let deadline_slack = requirement(ServiceRequirementSource::Flow, 6);
        let deadline_feasibility = requirement(ServiceRequirementSource::Flow, 7);
        let demand = ServiceDemand {
            identity: 11,
            requirement: service.identity,
            requirements: canonical_service_requirements([
                architecture,
                core_work,
                service,
            ...
            capacity_requirements: canonical_service_requirements([
                architecture,
                core_work,
                service,
                placement,
            ...
            response_requirements: canonical_service_requirements([
                architecture,
                core_work,
                service,
                placement,
            ...
            cancellation_requirements: canonical_service_requirements([
                architecture,
                core_work,
                service,
                placement,
        ...
        let mut problem = ServiceProblem {
            name: "exact_conflict_requirements",
            context: 7,
            assignment_fingerprint: 9,
            cores: vec![ServiceCoreResource {
                ordinal: 0,
        ...
        let ServiceSolveOutcome::Conflict(capacity) =
            solve_service_problem(&problem, &Cancellation::new()).unwrap()
        else {
            panic!("one-short capacity must conflict");
        };
        assert_eq!(capacity.code, ServiceConflictCode::CycleCapacity);
        assert_eq!(
            capacity.requirements.as_ref(),
            [architecture, core_work, service, placement, work_bound]
        );

        problem.cores[0].maximum_cycle_units = 4;
        problem.demands[0].maximum_delay_units = 5;
        let ServiceSolveOutcome::Conflict(deadline) =
            solve_service_problem(&problem, &Cancellation::new()).unwrap()
        else {
            panic!("one-short deadline must conflict");
        };
        assert_eq!(deadline.code, ServiceConflictCode::MaximumDelay);
        assert_eq!(
            deadline.requirements.as_ref(),
            [
                architecture,
                core_work,
                service,
    ...
    #[test]
    fn production_group_conflicts_partition_capacity_and_deadline_requirement_authority() {
        let assignment = assignment_from_foundation(fixture_from_source(
            "src/image.wr",
...
struct Worker:
    pub async fn run(self):
        mut group = actors.Group.all(bound=1u64)
        group.logical_deadline(epoch=1u64, slack=1000u64)
        _ = actors.Group.complete(take group)
...
"#,
        ...
        let translated = production_service_problem(&assignment, &Cancellation::new()).unwrap();
        let group = translated
            .demands
            .iter()
            .find(|demand| demand.kind == ServiceClassKind::GroupChild)
            .unwrap()
            .clone();
        let deadline_identities = assignment
            .flow_program
            .for_image_planning()
            .requirements()
            .filter(|requirement| {
            ...
            .map(|requirement| requirement.reference().identity())
            .collect::<BTreeSet<_>>();
        assert!(
            group
                .capacity_requirements
                .iter()
                .all(|reference| !deadline_identities.contains(&reference.identity))
        );
        assert!(deadline_identities.iter().all(|identity| {
            group
                .response_requirements
                .iter()
        ...
        let mut capacity_problem = translated.clone();
        capacity_problem.demands = vec![group.clone()];
        capacity_problem.cores = vec![ServiceCoreResource {
            ordinal: group.core,
            maximum_cycle_units: group.activation_units - 1,
        }];
        let ServiceSolveOutcome::Conflict(capacity) =
            solve_service_problem(&capacity_problem, &Cancellation::new()).unwrap()
        else {
            panic!("production Group capacity must conflict one unit short");
        };
        assert_eq!(capacity.requirements, group.capacity_requirements);
        verify_service_conflict(&capacity_problem, &capacity, &Cancellation::new()).unwrap();

        let mut deadline_problem = translated;
        let required_response = group
            .maximum_non_preemptive_units
            .checked_add(group.activation_units)
            .unwrap();
        let mut deadline_group = group;
        deadline_group.maximum_delay_units = required_response - 1;
        deadline_problem.demands = vec![deadline_group.clone()];
        deadline_problem.cores = vec![ServiceCoreResource {
            ordinal: deadline_group.core,
            maximum_cycle_units: deadline_group.activation_units,
        }];
        let ServiceSolveOutcome::Conflict(deadline) =
            solve_service_problem(&deadline_problem, &Cancellation::new()).unwrap()
        else {
            panic!("production Group deadline must conflict one unit short");
        };
