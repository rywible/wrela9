...
A Source Transaction names the source revision, opaque Semantic Identities, expected source provenance, and structural compatibility it inspected. Fingerprint equality is a fast acceptance case, not the authority: the compiler may rebase only within the selected host Project lineage when every intended target still resolves uniquely, its semantic path remains unambiguous, and its type, role, and interface remain compatible. Textual resemblance, inferred rename detection, and last-writer-wins behavior are insufficient. A deliberate atomic rename creates a new Semantic Identity and may carry an explicit old-to-new mapping only within that Transaction and its undo history; it creates no permanent source UUID or alias. Anonymous body sites must be recomputed after their owning body changes. Otherwise the compiler returns a structured conflict rather than a Creator language diagnostic.
