...
Function declarations state parameter modes and types, return type, generic parameters, and any explicit purity or effect ceiling. Local variables and closure details may be inferred. One narrow return-type exception exists: a private named function, async function, method, or associated function may write `Result[T]`, and the compiler infers exactly one Nominal Error from the error alternatives that flow out of that body. Public functions and every other declaration type spell `Result[T, E]`; Wrela has no anonymous or structural error-set type.

A Nominal Error is an ordinary named Wrela Data or Resource type in the error alternative of `Result`, not an exception class or special declaration kind. `Result[T, E]` inherits Resource ownership whenever either alternative owns Resources. A generic type parameter may occupy `E`, but each demanded concrete Specialization must substitute a nominal Data or Resource type; transparent aliases qualify through their normalized target without introducing an `Error` marker Interface. Private inference considers returned error alternatives and `?` propagation after compile-time selection, but not errors handled locally. Zero contributing types, multiple distinct types, or an unconstrained recursive cycle reject the omitted error annotation rather than manufacturing `Never` or an empty error type. Inference is independent of declaration order and caller expectations, solves recursive call groups together, and occurs independently after substitution for each demanded private generic Specialization.

A Nominal Error carries only the fields and ownership its declaration defines. `?` transfers that value unchanged and adds no implicit message, source trace, causal node, formatting, or runtime metadata. An explicit `Result[T, E]` is a permitted contract even when the current body produces no `E`, without an unused-error warning; reachability analysis may report or optimize impossible alternatives but cannot narrow the source type. A function value includes its resolved exact `Result[T, E]` return type. Functions with different error types cannot enter one finite callable family until ordinary explicit adapters map them to a shared Nominal Error.
...
One Project directory may build one Deployment Image rooted at `src/image.wr`, one Test Image rooted at `src/test.wr`, or both independently. Every other Project Module is nested beneath at least one directory. A repository may contain several independent Project directories, but Wrela has no manifest, multiple Deployment Images per Project, package, dependency, or external source model. The selected Project's absolute host path is operational metadata and cannot affect source semantics or Image identity.

Each nested regular `src/**/*.wr` file is a possible importable Module. Its canonical Project-relative path is its complete Module identity: `src/game/player.wr` defines `game.player`, while `src/image.wr` and `src/test.wr` define the distinguished non-importable root Modules `image` and `test`. Any other root-level `.wr` file is invalid Project topology rather than a one-segment Module. Source contains no redundant Module declaration, directory Module, index Module, or alternate file mapping. Every Module path segment matches `[a-z][a-z0-9_]*`; doubled and trailing underscores are valid spellings rather than parser-level style errors. Moving a file deliberately changes its Module identity.

Each selected root Module contains exactly one `@image` Image Constructor. `src/image.wr` selects a Deployment Image and `src/test.wr` selects a Test Image; a compilation selects one root and never merges their closures. Another `@image` in the selected reachable closure is rejected, while an unreachable source file is not part of that compilation. The filesystem adapter captures every regular `.wr` file beneath `src/` into the immutable Project snapshot, but the compiler analyzes only the selected root and its transitive import closure. Unreachable candidate source has no semantic diagnostics and cannot affect the accepted Image.

An import names one absolute complete Module and binds its namespace under the Module's leaf segment or an explicit `as` alias. Imports never copy declarations into the importing Module's unqualified scope. Every `pub` declaration in the imported Module is accessible through that namespace, while declarations and fields remain private to their defining Module unless marked `pub`. Every declaration identity exposed anywhere inside a public signature must itself be public and nameable from its defining Module; this applies recursively through generic arguments, function types, and `Result` alternatives. An imported public type may appear, but callers name it through its own defining Module because Wrela has no re-exports. Wrela has exactly one import form and initially has no relative, whole-path alternative, wildcard, selective-declaration, implicit-parent, friend, or re-export form. `pub` defines a source Module interface, not a compatibility or binary-ABI promise.
...
Module declarations are restricted to imports, `const`, `fn`, `async fn`, `suite`, `struct`, `resource struct`, `enum`, `interface`, `pool`, transparent `type` aliases, attributes, `comptime if`, and `comptime assert`; Modules have no runtime executable statements or mutable `static`. A Suite owns only nested `test` and `async test` declarations, takes no parameters, and has no runtime value. `@actor` classifies a struct as Actor state, while `@image` classifies an ordinary function as the one root Image Constructor. `const`, `comptime if`, and `comptime assert` are the complete initial compile-time syntax: ordinary pure functions execute during constant evaluation and Image construction without a separate `comptime fn` category. Every declaration and member is private unless `pub`, with no restricted visibility or public imports.
...
Every Actor in the sealed Image graph exists for the Image lifetime. Inactive behavior is explicit Actor state; handles never become stale through termination and Actors are not recreated dynamically. A Test Image uses the same one graph and lifetime rule; Wrela Tests neither create, reset, nor destroy Actors.
...
A sealed architecture primitive Adapter acknowledges hardware, captures bounded Untrusted device state, and wakes compiler-planned Driver or Facility work. Drivers are authenticated Wrela Modules that implement the substantial protocol and state-machine behavior through ordinary safe language operations plus narrowly granted Compiler Primitives. Trusted Facility Actors own Drivers and expose bounded high-level Data, Resources, and messages. Extending the Driver set requires an authenticated Compiler Distribution Module rather than ordinary Project source.

Creator standard-library Modules remain ordinary safe Wrela. Authenticated runtime and Driver Modules receive only the specific Compiler Primitives recorded for their exact Module identities; there is no general trusted language mode. The architecture primitive Adapters beneath those primitives are minimal Rust, assembly, or compiler-lowering Implementations for MMIO, interrupt and fault entry, barriers, context switching, and similarly irreducible effects. Creator source cannot import Drivers, request primitive authority, use FFI, declare raw pointers, or provide inline assembly.
...
Wrela has no `@budget` annotation that supplies or enforces a trusted work bound. A later Creator-facing ceiling may reject code whose independently proven work exceeds a stated contract, but it cannot replace the compiler's loop or recursion proof.
