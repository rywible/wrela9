...
Every Actor in the sealed Image graph exists for the Image lifetime. This includes Actors in dormant Test Case Graphs. Inactive behavior is explicit Actor state; handles never become stale through termination and Actors are not recreated dynamically.
