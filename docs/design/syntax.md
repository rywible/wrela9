...
The initial compiler-known `with` expression is the authenticated `core.pool.scoped(capacity=N)` factory. `N` is a compile-time `u64` maximum live-allocation count, not bytes or target layout. `with pools.scoped(capacity=4) as scratch:` binds `scratch` simultaneously as the fresh lexical Pool identity accepted by `own[scratch] T` and `Key[scratch, T]`, and as the scoped handle passed to authenticated Pool operations. The binding and every value parameterized by it are confined to the block. A Project declaration with the same spelling has no scope authority.

The first authenticated Pool operations, implemented with Layer 2 ownership and capacity semantics, have these source contracts after opening a scope `P`: `try_allocate[P, T](mut pool: Scoped[P], take value: T) -> Result[(own[P] T, Key[P, T]), PoolFull[T]]`; `lookup[P, T](read pool: Scoped[P], key: Key[P, T]) -> Option[T]` when `T` is Data; and `reclaim[P, T](mut pool: Scoped[P], take value: own[P] T) -> T`. `PoolFull[T].Full(value=T)` preserves the submitted value. `Key[P, T]` is copyable and unforgeable; lookup with a stale generation returns `Option.None`. Proof-dependent `allocate` and `reserve` join that interface only when whole-Image planning can admit them without changing source-visible types. Layer 1 seals and verifies the scope factory and lexical non-escape boundary without implementing allocation.
