...
On the Reference Development Host, representative warm local changes target a Check Latency below one second and have a hard five-second ceiling. Check measures its own latency. Crossing the ceiling fails on the reference host and warns on other hosts. When evidence growth threatens the envelope, cases are consolidated behind stronger invariants, deeper verifiers, or cheaper Regression Cases rather than moved into a slower routine tier.
...
Parser Regression Cases protect exact source-byte round trips, the physical-leaf partition, token and node kinds and ranges, trivia ownership, invalid and zero-width missing nodes, structured diagnostic and recovery actions, structurally valid declaration islands that remain eligible for semantic lowering, and the per-file diagnostic truncation rule. A compact corpus and bounded deterministic insert/delete/replace mutations cover every individual byte value, malformed-token extents, layout edges, recovery progress, later valid islands, and repeated-result determinism without running an exploratory fuzzer in Check. Most cases cross `Compiler::compile`; only low-level progress and recovery mechanics use the accepted private exception. They do not snapshot a printed private tree or parser-library representation. Broad fuzzing is a named sub-minute Challenge whose Findings reduce to narrow Regression Cases.
