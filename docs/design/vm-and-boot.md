...
Every member is hashed and one digest identifies the complete canonical Image. Mandatory distribution signatures are not part of the first format: local trust comes from the pinned toolchain, while a later publishing design may sign Image digests without changing guest semantics. The canonical Evidence Bundle is a separate host-side artifact keyed by the complete Image digest.

ELF is a host-side link and load envelope, not the Wrela Image abstraction. The Wrela launcher validates the container and supplies its members to pinned unmodified QEMU. Once QEMU has populated guest memory, the guest executes the VM ABI layout directly and never parses ELF or the `.wrela-image` container.

The Architecture Profile fixes the QEMU binary identity, versioned machine model, device order, virtual CPU count, exact RAM size, CPU-feature baseline, and canonical launch flags. The launcher does not discover a QEMU or linker from the host `PATH`, accept an unversioned machine alias, change RAM, or add devices outside the Device Manifest.
